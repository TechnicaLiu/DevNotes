# 运算题

1. 关于& || 的运算 

   ```js
     var a = 1,
         b = 0,
         c = true;
       console.log(a && b); // 0 
       console.log(a || b); // 1
       console.log(a && b && c); // 0
       console.log(a || b || c); // 1
       console.log(a + c);  // 2
   ```

   

# 执行顺序题

#### 宏任务和微任务

1. 执行原则  主线程=>微任务=>宏任务

   ```js
   // 注意： promise是同步任务 
   1.setTimeout(function(){
   2.    console.log('1')   // 宏任务
   3.});
   4.new Promise(function(resolve){
   5.    console.log('2');   // 主线程
   6.    resolve();
   7.}).then(function(){
   8.    console.log('3')   // 微任务
   9.});
   10.console.log('4');  // 主线程 
   11.new Promise(function(resolve){
   12.    console.log('5');   // 主线程
   13.    resolve();
   14.}).then(function(){
   15.    console.log('6')         // 微任务
   16.});
   17.setTimeout(function(){
   18.    console.log('7')  // 宏任务 
   19.});
   20.function bar(){
   21.    console.log('8')  // 主线程 
   22.    foo()
   23.}
   24.function foo(){
   25.    console.log('9')  // 主线程 
   26.}
   27.console.log('10')   // 主线程  
   28.bar()
   
   执行结果 2 4 5 10 8 9 3 6 1 7
   ```

2. 第二题

   ```js
   1.setTimeout(() => {
   2.      console.log('1');
   3.      new Promise(function (resolve, reject) {
   4.        console.log('2');
   5.        setTimeout(() => {
   6.          console.log('3');
   7.        }, 0);
   8.        resolve();
   9.      }).then(function () {
   10.        console.log('4')
   11.      })
   12.    }, 0);
   13.    console.log('5'); //5 7 10 8 1 2 4 6 3
   14.    setTimeout(() => {
   15.      console.log('6');
   16.    }, 0);
   17.    new Promise(function (resolve, reject) {
   18.      console.log('7');
   19.      // reject();
   20.      resolve();
   21.    }).then(function () {
   22.      console.log('8')
   23.    }).catch(function () {
   24.      console.log('9')
   25.    })
   26.    console.log('10');
   ```

   

# 数据结构

1. 某二叉查找树的每个节点存放一个整数,中序遍历该树得到的序列为3,4,5，则该树的画法有多少种情况?

   ![image-20211012101150473](../img/image-20211012101150473.png)

2. 

# JavaScript

1. 写一个函数，第一秒打印1，第二秒打印2

   * 使用let块级作用域

     ```js
     for(let i=0;i<5;i++){
         setTimeout(function(){
             console.log(i);
         },1000*i)
     }
     ```

     

   * 闭包

     ```js
     for(var i=0;i<5;i++){
         (function(i){
             setTimeout(function(){
                 console.log(i);
             },1000*i)
         })(i)
     }
     ```

     

2. 图片懒加载

   * 使用 元素的getBoundingClientRect().top 与浏览器的可视高度进行对比

     ```js
      <img data-src="../img/image.jpg" alt="" src= "">
         <script>
             const imgList = document.querySelectorAll('img')
             window.addEventListener('scroll',(e)=>{
                 imgList.forEach(image=>{
                     const imageTop = image.getBoundingClientRect().top;
                     if(imageTop < window.innerHeight){
                         const data_src =image.getAttribute('data-src');
                         image.setAttribute('src',data_src);
                     };
                     console.log('懒加载');
     
                 });
             });
         </script>
     ```

   * 新方法  IntersectionObservor   (有一定的浏览器兼容问题)

     ```js
     <img data-src="../img/image.jpg" alt="" src="">
         <script>
             const imglist = document.querySelectorAll('img');
     
             const callback = entries => {
                 entries.forEach(entry => {
                     if (entry.isIntersecting) {
                         const image = entry.target;
                         const data_src = image.getAttribute('data-src');
                         image.setAttribute('src', data_src);
                         observer.unobserve(image);
                     }
                 })
             }
     
             const observer = new IntersectionObserver(callback);
             imglist.forEach(image => {
                 observer.observe(image)
             })
         </script>
     ```

3. 打乱数组元素 

   思路： 数组从后往前，将目标元素和参考元素进行对调。

   ```js
   function unorderArr(students) {
       var length = students.length;
       var index;
       for (var point = length - 1; point >= 0; point--) {
           index = Math.floor(Math.random() * point); // 随机找一个数组元素
           temp = students[index];
           students[index] = students[point];
           students[point] = temp;
       }
       return students;
   }
   
   var students = ["学生1", "学生2", "学生3", "学生4", "学生5", "学生6", ];
   ```

   

4. 解析url 为指定对象

   思路： 利用数组的split 对 url 进行切割 ， 把url的参数循环遍历 依次存到对象中

   ```js
   // 已知网址 ： www.baidu.com?username=abin&userage=28&password=loveyou
   // 解析 上面的网址 ，返回的对象如下 ：
   /* obj={
       url:'www.baidu.com',
       params:{
           username1:'abin',
           userage:28,
           password:'loveyou'
       }
   } */
   
   function transformUrl(url) {
       var obj = {};
       obj.url = url.split('?')[0];
       obj.parameter = {};
       var paramsStr = url.split('?')[1];
       var paramsArr = paramsStr.split('&');
       for (let i = 0; i < paramsArr.length; i++) {
           var Pname = paramsArr[i].split('=')[0];
           var Pvalue = paramsArr[i].split('=')[1];
           obj.parameter[Pname] = Pvalue
       }
       return obj;
   }
   var url = 'www.baidu.com?username=abin&userage=28&password=loveyou'
   
   var result=transformUrl(url);
   
   ```

5. 用setTimeout 模拟实现  setInterval  

   ```js
   function newInterval(func,millsecond){
       function inside(){
           func();
           setTimeout(inside , millsecond);
       }
       setTimeout(inside,millsecond);
   }
   newInterval(like ,1000);
   function like(){
       console.log('记得点赞哦');
   }
   ```

   

6. 手写call()方法 

   实现思路：

   * 判断是否是函数调用，如果不是则直接抛出异常
   * 通过新对象 context来调用函数
   * 给context 创建一个fn设置为需要调用的函数
   * 结束调用之后删除fn 

   ```js
   Function.prototype.call1 = function (context) {
     // 先判断 调用call1是不是一个函数 
     if (typeof this !== 'function') {
       throw new TypeError('Not a Function ')
     }
     // 不传参默认为window  
     context = context || window
     // 保存this 
     context.fn = this
     // 保存参数
     let args = Array.from(arguments).slice(1); // 将伪数组转换为数组 
     // 调用函数 
     let result = context.fn(...args);  
     delete context.fn
     return result
   }
   ```

7. 手写apply()方法

   ```js
   Function.prototype.apply1=function(context){
      if(typeof this !== 'function'){
        throw new TypeError('Not a Function');
      }
      let result;
      context=context || window ;
      context.fn=this;
      if(arguments[1]){
        result= context.fn(...arguments[1]);
      }else{
        result=context.fn()
      }
      delete context.fn
      return result 
   }
   ```

8. 手写bind()方法 

   实现思路：

   * 在内部直接调用apply方法

   ```js
   Function.prototype.mybind=function(context){
     // 判断调用bind的是否是函数
     if(typeof this!=='function'){
       throw new TypeError('Not a function');
     }
     // 保存调用bind的函数 
     let that =this ;
     // 拿到拿到 Fn.mybind(o,1,2)(3) 中的 1，2 参数参数 
     let args1=Array.prototype.slice.call(arguments,1),args2=null;
     return function (){
        // 拿到 Fn.mybind(o,1,2)(3) 中的 3 参数
         args2=Array.prototype.slice.call(arguments);
         // 使用apply来改变this ,进行调用 
        that.apply(context,args1.concat(args2))
       } 
   }
   ```

9. 利用递归求数组的和

   ```js
   let arr = [10, 100, 1000, 10000];
   function sum(array) {
     if (array.length === 1) {
       return array[0];
   
     } else {
       return array[array.length - 1] + sum(array.slice(0, array.length - 1));
     }
   }
   console.log(sum(arr));
   ```

   